...
export class UpdateUserProfileService {
  ...
  async run(
    ...
    await this.user_profile_repository.updateByUserId(target_id, data);
  }
  ...
  private authorize(user: TRequestUser, target_id: string): void {
    ...
    const permissions =
      user.id === target_id
        ? acl_query.updateOwn(AUTHORIZATION_RESOURCE.PROFILE)
        : acl_query.updateAny(AUTHORIZATION_RESOURCE.PROFILE);
